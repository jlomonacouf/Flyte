...
import { backendEndpoint, USER_FOLLOWING_URL} from "../../src/api_methods/shared_base";
...
async componentDidMount(){
    ...
    if(this.props.route.params!=undefined) fetchID=this.props.route.params.username; //Get username of user we want to view 
      ...
      const information = await fetch(backendEndpoint + USER_FOLLOWING_URL + fetchID, requestOptions)
      ...
      .then( result => {  
          if(result.success){
            ...
            this.setStateAsync({ followingList : result.results});
          }else{
  ...
  renderFollowing = (follower, index) => {
    ...
    return (
      ...
      <TouchableWithoutFeedback style={{zIndex:3}} key={`user-${follower.username}`} onPress={() => navigation.navigate("Profile", {view_username: follower.username})}  >
    ...
    return (
      <Block flex style={styles.group}>
        ...
        <Block style={{ width: width - theme.SIZES.BASE * 2, paddingHorizontal: theme.SIZES.BASE}}>
          <ScrollView>
            { this.state.followingList && this.state.followingList.map((follower,index) =>
